/**
 * Typed tactic & wildcard catalog for the playable simulation (single source of truth).
 */
...
export const TACTIC_CATALOG: EnrichedTactic[] = [
...
export type SimulationPlayQuarter = "Q1" | "Q2" | "Q3" | "Q4";

/**
 * Quarter tactic rosters previously expressed as `TACTIC_CATALOG.slice(start, end)`.
 * IDs must exist on {@link TACTIC_CATALOG}.
 */
export const QUARTER_TACTIC_IDS: Record<SimulationPlayQuarter, readonly string[]> = {
  Q1: ["digital-1", "digital-2", "digital-3", "content-1", "content-2", "content-3"],
  Q2: ["content-2", "content-3", "traditional-1", "traditional-2", "events-1", "events-2"],
  Q3: ["events-1", "events-2", "partnerships-1", "partnerships-2", "digital-4", "content-4"],
  Q4: ["digital-4", "content-4", "traditional-3", "events-3", "partnerships-3", "digital-5"],
};

const tacticCatalogById = new Map(TACTIC_CATALOG.map((tactic) => [tactic.id, tactic]));

export function getTacticsAvailableInQuarter(quarter: SimulationPlayQuarter): EnrichedTactic[] {
  return QUARTER_TACTIC_IDS[quarter]
    .map((id) => tacticCatalogById.get(id))
    .filter((tactic): tactic is EnrichedTactic => tactic !== undefined);
}

export function getTacticsByCategory(category: Tactic["category"]): EnrichedTactic[] {
  return TACTIC_CATALOG.filter((tactic) => tactic.category === category);
}
...
export function getTacticById(id: string): EnrichedTactic | undefined {
  return tacticCatalogById.get(id);
}
