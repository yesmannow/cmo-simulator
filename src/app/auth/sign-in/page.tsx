...
function SignInForm() {
  ...
  const handleGoogle = async () => {
    ...
    setIsLoading(true);
    try {
      await signInWithGoogleOAuth(nextPath);
    } catch (error) {
      setMessage(formatAuthErrorMessage(error), 'error');
      setIsLoading(false);
    }
      ...
      <section className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5">
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-800 shadow-sm hover:bg-slate-50 disabled:opacity-60"
          >
          ...
          <p className="mt-2 text-center text-xs text-slate-500">or continue with email</p>
        </div>

        <form className="mt-4 space-y-3" onSubmit={handleSubmit} autoComplete="on">
          <div>
